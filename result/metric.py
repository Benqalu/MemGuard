...
class Metric(object):
	...
	def accuracy_disparity(self,s,absolute=True):
		metric1=Metric(true=self.true[s==0],pred=self.pred[s==1])
		metric2=Metric(true=self.true[s==0],pred=self.pred[s==1])
		if absolute:
			return abs(metric1.recall()-metric2.recall())
		else:
			return metric1.recall()-metric2.recall()
